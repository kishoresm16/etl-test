...
TEXT_EXTENSIONS = (".php", ".xml", ".csv", ".json", ".txt", ".log", ".sql", ".bak", ".html")
...
CLOUD_MISCONFIG_PATTERNS = {
    "s3_bucket_url": r"https?://[a-zA-Z0-9\-\.]*s3\.amazonaws\.com/[^\s\"']+",
    "gcp_storage": r"https?://storage\.googleapis\.com/[^\s\"']+",
    "azure_blob": r"https?://[a-z0-9]{3,24}\.blob\.core\.windows\.net/[^\s\"']+",
    "firebase_db": r"https?://[a-zA-Z0-9-]+\.firebaseio\.com[^\s\"']*",
    "ec2_metadata_url": r"http://169\.254\.169\.254/[^\s\"']*",
    "cloudfront_url": r"https?://[a-z0-9]+\.cloudfront\.net[^\s\"']*",
    "aws_arn": r"arn:aws:[^:]+:[^:]*:[0-9]*:[^\"'\s]+",
    "gcp_project_id": r"projects/[a-zA-Z0-9\-_]+"
}
...
def contains_sensitive_data(text):
    return (
        any(re.search(pattern, text) for pattern in SENSITIVE_PATTERNS)
        or any(re.search(pattern, text) for pattern in CLOUD_MISCONFIG_PATTERNS.values())
    )
